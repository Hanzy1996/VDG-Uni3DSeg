...
#  'ViT-L/14',
#  'ViT-L/14@336px']
hparams['dataset'] = 'scannet200'
hparams['gpt_type'] = 'llama_v3'
...
# I recommend using VISSL https://github.com/facebookresearch/vissl/blob/main/extra_scripts/README.md to download these
    ...
    classes_to_load = None
elif hparams['dataset'] == 's3dis':
    hparams['data_dir'] = pathlib.Path(DTD_DIR)
    dataset = None
    hparams['descriptor_fname'] = 's3dis'+'_' + hparams['gpt_type']
    classes_to_load = None
elif hparams['dataset'] == 'scannet':
    hparams['data_dir'] = pathlib.Path(DTD_DIR)
    dataset = None
    hparams['descriptor_fname'] = 'scannet' + '_' + hparams['gpt_type']
    classes_to_load = None
elif hparams['dataset'] == 'scannet200':
    hparams['data_dir'] = pathlib.Path(DTD_DIR)
    dataset = None
    hparams['descriptor_fname'] = 'scannet200' + '_' + hparams['gpt_type']
    classes_to_load = None
...
# gpt_descriptions, unmodify_dict = load_llama_v2_descriptions(hparams, classes_to_load)

gpt_descriptions, unmodify_dict = load_llama_v3_descriptions(hparams, classes_to_load)
...
def compute_label_encodings(model):
    label_encodings = (model.encode_text(clip.tokenize([hparams['label_before_text'] + wordify(l) + hparams['label_after_text'] for l in label_to_classname]).to(hparams['device'])))
