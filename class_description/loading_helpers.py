...
def load_llama_descriptions(hparams, classes_to_load=None):
    llama_descriptions_unordered = load_json(hparams['descriptor_fname'])
    unmodify_dict = {}
    if classes_to_load is not None: 
        llama_descriptions = {c: llama_descriptions_unordered[c] for c in classes_to_load}
    else:
        llama_descriptions = llama_descriptions_unordered

    if hparams['category_name_inclusion'] is not None:
        if classes_to_load is not None:
            keys_to_remove = [k for k in llama_descriptions.keys() if k not in classes_to_load]
            for k in keys_to_remove:
                print(f"Skipping descriptions for \"{k}\", not in classes to load")
                llama_descriptions.pop(k)
            
        for i, (k, v) in enumerate(llama_descriptions.items()):
            v_concat = v['high_level_description'][:5] + v['mid_level_description'][:5] + v['low_level_description'][:5]
            ...
            unmodify_dict[k] = {build_descriptor_string(item): item for item in v_concat}
                
            llama_descriptions[k] = [build_descriptor_string(item) for item in v_concat]
            
            # print an example the first time
            if i == 0: #verbose and 
                print(f"\nExample description for class {k}: \"{llama_descriptions[k][0]}\"\n")
    return llama_descriptions, unmodify_dict



def load_llama_v2_descriptions(hparams, classes_to_load=None):
    llama_descriptions_unordered = load_json(hparams['descriptor_fname'])
    unmodify_dict = {}
    if classes_to_load is not None: 
        llama_descriptions = {c: llama_descriptions_unordered[c] for c in classes_to_load}
    ...
    if hparams['category_name_inclusion'] is not None:
        ...
        for i, (k, v) in enumerate(llama_descriptions.items()):
            v_concat = v['semantic_description'][:5] + v['spatial/geometric_description'][:5]
...
def load_llama_v3_descriptions(hparams, classes_to_load=None):
    llama_descriptions_unordered = load_json(hparams['descriptor_fname'])
    unmodify_dict = {}
    if classes_to_load is not None: 
        llama_descriptions = {c: llama_descriptions_unordered[c] for c in classes_to_load}
