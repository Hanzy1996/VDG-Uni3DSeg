...
class ScanNetSemanticCriterion:
    ...
    def __call__(self, pred, insts):
        ...
        for pred_mask, gt_mask in zip(pred['sem_preds'], insts):
            ...
            losses.append(F.cross_entropy(pred_mask, gt_mask.sp_masks.float().argmax(0), ignore_index=self.ignore_index))
...
@MODELS.register_module()
class S3DISSemanticCriterion_img_split:
    """Semantic criterion for S3DIS.
    ...
    def get_layer_loss(self, layer, aux_outputs, insts):
        ...
        # seg_losses = []
        for pred_mask, gt_mask in zip(pred_masks, insts):
            des_pred_mask, img_pred_mask = pred_mask.chunk(2, dim=0)

            des_seg_loss = self.seg_loss(des_pred_mask.T, gt_mask.sp_masks.float().argmax(0))
            des_seg_losses.append(des_seg_loss)

            img_seg_loss = self.seg_loss(img_pred_mask.T, gt_mask.sp_masks.float().argmax(0))
            img_seg_losses.append(img_seg_loss)

        #     sem_pred = des_pred_mask + img_pred_mask
        #     seg_loss = self.seg_loss(sem_pred.T, gt_mask.sp_masks.float().argmax(0))
        #     seg_losses.append(seg_loss)
        # seg_loss = self.loss_weight * torch.mean(torch.stack(seg_losses))
        # return {f'layer_{layer}_seg_loss': seg_loss}

        des_seg_loss = self.loss_weight * torch.mean(torch.stack(des_seg_losses))
        img_seg_loss = self.loss_weight * torch.mean(torch.stack(img_seg_losses))
        return {f'layer_{layer}_des_seg': des_seg_loss, f'layer_{layer}_img_seg': img_seg_loss}
    ...
    def __call__(self, pred, insts):
        ...
        loss = {'last_layer_des_seg': des_seg_loss, 'last_layer_img_seg': img_seg_loss}
        # loss = {'last_layer_seg_loss': seg_loss}
...
@MODELS.register_module()
class S3DISSemanticCriterion_v2:
    """Semantic criterion for S3DIS.
    ...
    def get_layer_loss(self, layer, aux_outputs, insts):
        ...
        for pred_mask, gt_mask in zip(pred_masks, insts):
            seg_loss = self.seg_loss(
                pred_mask.T, gt_mask.sp_masks.float().argmax(0), ignore_index=self.ignore_index)
            seg_losses.append(seg_loss)
...
@MODELS.register_module()
class S3DISSemanticCriterion_han:
    """Semantic criterion for S3DIS.
