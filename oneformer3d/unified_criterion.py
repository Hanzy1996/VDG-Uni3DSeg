...
class ScanNetUnifiedCriterion_contrast:
    ...
    def __init__(self, num_semantic_classes, sem_criterion, inst_criterion, contrast_criterion):
        self.num_semantic_classes = num_semantic_classes
        self.sem_criterion = MODELS.build(sem_criterion)
        self.inst_criterion = MODELS.build(inst_criterion)
        self.contrast_criterion = MODELS.build(contrast_criterion)
    ...
    def __call__(self, pred, insts):
        ...
        loss.update(self.contrast_criterion(pred['sem_attns'],  sem_gts))
        return loss



@MODELS.register_module()
class S3DISUnifiedCriterion_contrast:
    """Simply call semantic and instance criterions.
...
@MODELS.register_module()
class ScanNetUnifiedCriterion_han:
    """Simply call semantic and instance criterions.
    ...
    def __call__(self, pred, insts):
        ...
        for i in range(len(pred['masks'])):
            ...
            if insts[i].get('query_masks') is not None:
                ...
                sem_gt.sp_masks = insts[i].sp_sem_masks
            sem_gts.append(sem_gt)
            sem_preds.append(pred['masks'][i][-n:, :])
        ...
        loss = self.inst_criterion(pred, inst_gts)
        loss.update(self.sem_criterion({'masks': sem_preds, 'aux_outputs': sem_aux_outputs}, sem_gts))
        return loss
