...
class HungarianMatcher_han:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        cost_values = []
        # pred_instances.scores.shape  torch.Size([400, 19])
        # gt_instances.labels.shape   torch.Size([39])
        for cost in self.costs:
            cost_values.append(cost(pred_instances, gt_instances))
...
class OneDataCriterion_v2:
    ...
    def __init__(self, matcher, loss_weight, non_object_weight, 
                 num_classes_dataset, fix_dice_loss_weight, iter_matcher):
        self.matcher = TASK_UTILS.build(matcher)
